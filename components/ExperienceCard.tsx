  ...
  return (
    <div className="px-4 md:px-0">
      <div
        ...
        className={`group border border-white/10 bg-transparent backdrop-blur-xl overflow-hidden transition-all duration-150 cursor-pointer hover:border-white/20 ${getRoundedClass(
          rounded
        )}`}
      >
        {/* HEADER */}
        <div className="flex justify-between items-start md:items-center p-4 md:p-5 gap-3">
          <div className="flex items-start md:items-center gap-3 md:gap-4 min-w-0">
            ...
            <div className="flex flex-col justify-center min-w-0">
              <h2 className="text-lg md:text-2xl font-semibold tracking-tight leading-snug flex flex-wrap items-center gap-2 break-words text-white">
                <span className="truncate">{company}</span>
                <span className="text-[10px] md:text-sm text-white/60 font-normal border border-white/15 px-2 py-0.5 rounded-sm whitespace-nowrap">
                  {role}
                </span>
              </h2>

              <div className="text-white/60 text-xs md:text-sm leading-tight">
                {duration}
              </div>
            </div>
          </div>
        ...
        >
          <div
            ref={contentRef}
            className={`px-4 md:px-5 pb-4 md:pb-5 text-sm md:text-base text-white/80 leading-relaxed transition-all duration-500 ${
              open
            ...
            <div className="space-y-1.5 md:space-y-2">
              {points.map((point, index) => (
                <p key={index} className="relative pl-4">
                  <span className="absolute left-0 top-[8px] h-1 w-1 rounded-full bg-white/60" />
                  {point}
                </p>
              ))}
            </div>
